...
const getAllAttemptsByUserId = (user_id) => {
  return db
    .query("SELECT * FROM attempts WHERE user_id = $1", [user_id])
    .then((data) => {
      return data.rows;
    })
    .catch((err) => {
...
const getAttemptsByDate = (user_id, date) => {
  return db
    .query("SELECT * FROM attempts WHERE user_id = $1 AND date = $2", [
      user_id,
      date,
    ])
    .then((data) => {
...
const getQuizIdByAttempt = (attempt_id) => {
  return db
    .query("SELECT quiz_id FROM attempts WHERE id = $1", [attempt_id])
    .then((data) => {
      return data.rows;
    })
    .catch((err) => {
...
const getUserIdByAttempt = (attempt_id) => {
  return db
    .query(
      "SELECT users.username FROM attempts JOIN users ON user_id = users.id WHERE attempts.id = $1",
      [attempt_id]
    )
    .then((data) => {
      return data.rows[0];
